...
fn get_attendees(users: List(User), ids: List(Int)) -> List(Attendee) {
  list.flat_map(ids, fn(id) { list.filter(users, fn(user) { user.id == id }) })
  |> list.map(fn(user) { Attendee(user: user, accepted: True) })
}
